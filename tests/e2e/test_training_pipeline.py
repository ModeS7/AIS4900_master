    ...
    def test_resume_training(self, temp_output_dir):
        ...
        resume_run = run_training_command(
            [
                ...
                "training.use_compile=false",
                "training.warmup_epochs=0",
                f"training.resume_from={checkpoint_path}",
    ...
    def test_vqvae_training_completes(self, temp_output_dir):
        ...
        result = run_training_command(
            [
                ...
                "training.use_compile=false",
                "training.warmup_epochs=0",
                "vqvae.channels=[32,64]",
                "vqvae.attention_levels=[false,false]",
                "vqvae.num_res_blocks=1",
                f"hydra.run.dir={temp_output_dir}",
            ],
            timeout=300,
            check=False,
    ...
    def test_mode_training_starts(self, mode, temp_output_dir):
        ...
        result = run_training_command(
            [
                ...
                "training.epochs=1",
                "training.batch_size=2",
                "training.limit_train_batches=3",
                "training.generation_metrics.enabled=false",
                "training.use_compile=false",
                "training.warmup_epochs=0",
