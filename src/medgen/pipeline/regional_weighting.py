...
Computes per-pixel weight maps based on tumor size using RANO-BM clinical
...
class RegionalWeightComputer:
    """Compute per-pixel weight maps based on tumor size.
    ...
    Args:
        image_size: Image size in pixels (e.g., 256).
    ...
    Example:
        >>> computer = RegionalWeightComputer(image_size=256)
        >>> mask = torch.zeros(2, 1, 256, 256)  # Batch of 2
    ...
    def __call__(self, mask: Tensor) -> Tensor:
        """Compute per-pixel weight map from segmentation mask.

        Args:
            mask: Binary segmentation mask [B, 1, H, W] or [B, H, W].

        Returns:
            Weight map [B, 1, H, W] with same dtype and device as input.
        """
...
def create_regional_weight_computer(cfg) -> Optional[RegionalWeightComputer]:
