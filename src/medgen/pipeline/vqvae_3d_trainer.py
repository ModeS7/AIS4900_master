...
class VQVAE3DTrainer:
    ...
    def _save_checkpoint(self, epoch: int, name: str) -> None:
        ...
        # Build extra state for VQ-VAE specific components
        extra_state = {
        ...
        for modality, loader in self.per_modality_val_loaders.items():
            ...
            with torch.no_grad():
                for batch in loader:
                    ...
                    with autocast('cuda', enabled=True, dtype=self.weight_dtype):
                        reconstruction, _ = model_to_eval(images)
    ...
    def evaluate_test(
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Test evaluation", ncols=100, disable=self.is_cluster):
                ...
                with autocast('cuda', enabled=True, dtype=self.weight_dtype):
                    reconstruction, _ = model_to_use(images)
    ...
    def _create_test_reconstruction_figure_3d(
        ...
        title_parts = [f"3D VQ-VAE Test Reconstructions ({label})"]
        metric_strs = []
        if 'psnr' in metrics:
