...
| 61 | Multiple ODE Solvers | Euler, midpoint, heun, RK4, dopri5 via torchdiffeq | Inference-time solver selection. Euler/27 optimal (ImageNet FID); all higher-order solvers worse | Various |
| 65 | Time-Shifted Sampler | Adjusts step schedule based on resolution — shifts more steps to mid-range timesteps | Inference-only, drop-in replacement for step schedule. Shift ratio 2.0 optimal (FID 49.25 vs 54.78@1.0); see proven_techniques.md | ICLR 2024 |
...
|---|-------|-------------|---------------|--------|-----------|
| 110 | **FreeU (Skip Connection Reweighting)** | Inference-only: amplifies UNet backbone features (factor b>1) and attenuates high-frequency skip connections (spectral filter factor s<1). Two scalars per decoder level | **EVALUATED**: Marginal benefit. Best config (b=1.0, s=0.9) gave FID=19.87 vs baseline 19.12. Not worth the complexity. See `find_optimal_freeu.py` and experiment_results_3d.md Part 14 | Trivial (~10 lines in sampling, no retraining) | Si et al., CVPR 2024. [arXiv:2309.11497](https://arxiv.org/abs/2309.11497) |
...
| 117 | **Diffusion Mixup** — ✅ SINCE IMPLEMENTED (`diffusion_mixup` config + `_apply_diffusion_mixup`, trainer.py; found unhelpful in latent, exp28_1) | Interpolate pairs of training volumes in pixel space before adding noise: `x_mix = λ*x_a + (1-λ)*x_b`. Model denoises from interpolated examples on the convex hull of data manifold | Creates genuinely unseen training samples via convex interpolation. Different from CutMix/Mosaic (#24, compression only). Score smoothing analysis (2025) shows diffusion models already implicitly interpolate — making it explicit strengthens anti-memorization | Low (~10 lines) | On the Interpolation Effect of Score Smoothing, 2025. [arXiv:2502.19499](https://arxiv.org/html/2502.19499) |
...
| Category | Count |
|----------|-------|
| Implemented | 69 (68 functional + 1 dead config) — incl. #114 TADA + #117 Diffusion Mixup, still listed under MEDIUM below but since implemented |
| Not implemented — HIGH (novel axis, strong potential) | 4 (#110–#113) |
| Not implemented — MEDIUM (worth trying) | 5 (#68, #115, #116, #118, #119); #114 + #117 since implemented (see row notes) |
